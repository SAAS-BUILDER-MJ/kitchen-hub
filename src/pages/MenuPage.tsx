...
const MenuPage = () => {
  ...
  const filtered = menuItems.filter((m) => {
    const matchesCategory = activeCategory === 'All' || m.category_name === activeCategory;
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      <header className="sticky top-0 z-30 bg-background/80 backdrop-blur-md border-b px-4 py-3">
        ...
        {showSearch && (
          <div className="max-w-2xl mx-auto mt-2">
            <Input placeholder="Search menu items..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-9" autoFocus />
          </div>
        )}
      </header>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-3">
        {filtered.length === 0 && <p className="text-center py-12 text-muted-foreground">No items found</p>}
        {filtered.map((item) => (
          <MenuItemCard
            key={item.id}
            item={item}
            quantity={getCartQty(item.id)}
            onAdd={() => addToCart({ id: item.id, name: item.name, price: item.price, emoji: item.emoji })}
            onRemove={() => {
      ...
      {cartCount > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/90 backdrop-blur-md border-t animate-slide-up">
          <div className="max-w-2xl mx-auto">
            <Button className="w-full py-6 text-base font-semibold" onClick={() => navigate('/cart')}>
  ...
  <div className={`flex items-center gap-4 p-4 bg-card rounded-lg border shadow-sm animate-slide-up ${!item.available ? 'opacity-50' : ''}`}>
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-center gap-2">
        ...
        {!item.available && (
          <Badge variant="outline" className="text-[10px] bg-destructive/10 text-destructive border-destructive/30">Unavailable</Badge>
        )}
      </div>
    ...
    <div>
      {!item.available ? (
        <Button size="sm" variant="outline" disabled className="opacity-50">Unavailable</Button>
      ) : quantity === 0 ? (
